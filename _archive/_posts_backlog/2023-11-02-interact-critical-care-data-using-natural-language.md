...
However, there is one area where I can see immediate gains in productivity: how users interact with data.  As much of healthcare data is now stored in EHRs due to the HITECH act, it has inadvertently become more cumbersome to go through numerous clicks in an EHR system, or rely on a data analyst to get information.  Thanks to LLMs, any user can now converse with a database using natural language.  This is an exciting and powerful new capability for anyone that works with healthcare data.
