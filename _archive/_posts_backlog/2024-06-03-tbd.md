...
At a high level, supervised learning uses labeled inputs and outputs to fit a model.  Here, the machine algorithm learns the best mapping function from input to output.  Unsupervised learning differs in that there are no labeled inputs nor outputs.  Here, the machine learning algorithm learns the best mapping function that finds similarity among the input samples, and groups them based on that similarity as the output.  
