...
One of the first challenges seen with one-hot vectors is that no information can be gleaned from the words they represent, or how the words relate to each other.  If we were to graph each word in a 14-dimensional space and calculate the Euclidean distance between each vector, we would see that... 
