...
Each word has to be represented as a one-hot vector of the dimension 14 (vocabulary size).  That is, each word in a document is represented as its own single line with '1' representing the unique word, and '0' representing all other words.  I think of it as flattening out each unique word.  We have something that looks like this:

![encoding](/assets/images/2023-07-28_encoding.png)

## Why does my data look sparse?

When we replace each word in document with these one-hot vectors, we start to get a large and sparse space with a lot of zeroes.  In fact, when we start to increase our vocabulary size (keep in mind, we started with just 5 sentences across 3 documents), our data starts to look something like this:
