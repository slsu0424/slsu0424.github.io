...
Now, when we replace each word in document with these one-hot vectors, we start to get a large and sparse space with a lot of zeroes.  In fact, when we start to increase the dimension, we start to get something that starts to look like this:
