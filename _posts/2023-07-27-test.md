...
Each word is represented as a one-hot vector of the dimension 14 (vocabulary size).  That is, each word in a document is represented with a '1' corresponding to the unique word, and '0' representing all other words.  We have something that looks like this:

![encoding](/assets/images/2023-07-28_encoding.png)

## Limitations of one-hot vectors

One of the first challenges seen with one-hot vectors is that they know nothing about the words they represent or how the words relate to each other.  Graphically, if we were to graph each word, we would see that 


Another challenge is sparse data.  When we replace each word in document with these one-hot vectors, we start to get a large and sparse space with a lot of zeroes.  In fact, when we start to increase our vocabulary size (keep in mind, we started with just 5 sentences across 3 documents!), our data will morph into something like this:

![encoding_sparse](/assets/images/2023-08-02_encoding_sparse.png)


This data representation makes it difficult for a machine to detect any sort of meaningful patterns.
