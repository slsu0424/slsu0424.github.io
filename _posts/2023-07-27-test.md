...
In this tutorial, we considered the one-hot encoding technique, a very simple way to encode categorical variables for natural language processing tasks.  Each word is encoded as a binary vector, with '1' representing the unique word and '0' representing all others.  By taking a subset of PubMed abstracts that discussed cardiovascular disease (CVD) risk factors, we were able to see how this approach becomes highly inefficient as larger vocabularies are considered.
