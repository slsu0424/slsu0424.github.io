...
Another challenge is sparse data.  When each word in a document is replaced with a one-hot vectors, we start to get a large space with a lot of zeroes.  In fact, when increase our vocabulary size (keep in mind, we started with just 5 sentences across 3 documents!), our word representation will morph into something like this:
