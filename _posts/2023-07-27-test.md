...
When we replace each word in document with these one-hot vectors, we start to get a large and sparse space with a lot of zeroes.  In fact, when we start to increase our vocabulary size (keep in mind, we started with just 5 sentences across 3 documents!), the representation of the data will morph into something like this:
