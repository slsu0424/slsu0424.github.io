...
Under the hood, we pass in the OpenAI model (gpt-3.5-turbo), and set the [temperature](https://platform.openai.com/docs/guides/gpt/how-should-i-set-the-temperature-parameter).  The temperature controls the randomness of the output generated by the model (closer to 1 will generate a more creative response).  For [chain_type = 'stuff'](https://python.langchain.com/docs/modules/chains/document/stuff), this will combine the query (question), along with the relevant documents (context) into a single prompt to pass to the LLM.
...
[pyodbc](https://pypi.org/project/pyodbc/) is used to connect python to Azure SQL DB via [pyodbc](https://pypi.org/project/pyodbc/).  First, wew get the database connection string variables:
