...
Healthcare is inundated with data - much of it is in unstructured formats such as EHR notes, radiology images, medical device readings, and even consumer fitness wearables.  Focusing on the former, there is a wealth of knowledge buried in pages upon pages of written text.  While processing lengthy documents are nothing new for computers, retrieving relevant information using NLP techniques have been less than stellar.  For example, the user may have to be explicit in their query statement to retrieve the correct information.
