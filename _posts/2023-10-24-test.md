...
This [blogpost](https://github.com/hwchase17/chat-your-data/blob/master/blogpost.md) by LangChain's founder provides a good overview for building a text-based Q&A application.  
...
The VectorStoreIndexCreator handles steps 2-4 above (chunking, embedding, and storage).  This [article](https://medium.com/@kbdhunga/enhancing-conversational-ai-the-power-of-langchains-question-answer-framework-4974e1cab3cf) goes into a bit more detail about the class.  Some default settings to note are: 
...
- Embedding - The OpenAI API [OpenAIEmbeddings](https://api.python.langchain.com/en/latest/embeddings/langchain.embeddings.openai.OpenAIEmbeddings.html) is used to generates embeddings
- Storage - embedding vectors are stored in the open-source vector store [Chroma](https://www.trychroma.com/)
