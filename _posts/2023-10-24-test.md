...
Pre-requisites:
1. Familiarity with OpenAI
2. Familiarity with Azure data services
3. Python
...
The VectorStoreIndexCreator handles steps 2-4 above (chunking, embedding, and storage).  This [article](https://medium.com/@kbdhunga/enhancing-conversational-ai-the-power-of-langchains-question-answer-framework-4974e1cab3cf) goes into more detail about the class, including customization.  Some default settings to note are: 
...
- Embedding - uses [OpenAIEmbeddings](https://api.python.langchain.com/en/latest/embeddings/langchain.embeddings.openai.OpenAIEmbeddings.html) to generates embeddings
...
Next, we setup a simple external UI to allow users to ask questions of the ADE document.  
...
The MIMIC-III is an open database comprising de-identified health-related data for > 40,000 patients who stayed in critical care units of the Beth Israel Deaconess Medical Center between 2001 and 2012.  I used the MIMIC-III database to  
